...
// Return 1 (true) if both trees are identical; otherwise return 0 (false).
// This function is implemented recursively.
// Base cases must meet the following conditions:
//  - Neither tree1 nor tree2 is NULL.
//  - All node values in tree1 and tree2 must be identical.
//  - Tree1 and tree2 must have the same structure.
int identical(BTNode *tree1, BTNode *tree2)
{
    ...
    //       One tree has nodes while the other is empty.
    //       Trees are not structurally identical.
    //       Return 0 (false).
    if (tree1 == NULL || tree2 == NULL)
        return 0;
    // bellow here...
    // Case: Compare current node values and recurse on subtrees.
    //       Return 1(true) only if "items match and both subtrees are identical".
