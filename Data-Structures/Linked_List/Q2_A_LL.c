...
// 현재 내가 보고 있는 ll2의 index에 해당하는 노드를 ll2의 index+1에 삽입하는 느낌?
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
	int index = 0;
	ListNode *curr_ll1;
	ListNode *curr_ll2;
	curr_ll1 = ll1->head;
	curr_ll2 = ll2->head; // ll1로 삽입할 현재 노드를 가리키는 포인터

	while (curr_ll2 != NULL)
	{
		insertNode(ll1, 2 * index + 1, curr_ll2->item);

		curr_ll1 = curr_ll1->next->next;
