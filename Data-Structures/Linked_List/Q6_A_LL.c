...
// head는 값이다
// head를 담는 공간을 넘겨준다
// 값만 바꾸면 head가 가리키는 값은 안 바뀐다
// 공간을 넘겨줘야 head가 가리키는 값이 바뀐다
// ListNode(**) 이렇게 볼 게 아니라 (ListNode*)*로 생각해라
// LinkedList * 이랑 (ListNode*)* 같은 말이다
// LinkedList내에 head가 있기 때문에 다시 생각해라
